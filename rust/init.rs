...
/// Keep session **seeds** and session-local state out of git. The key file
/// (`*.key`) is secret and is minted/shredded per session; a session's identity
/// belongs to the session rather than the repository. The store is committed.
const GITIGNORE: &str = r#"# Session signing seeds never leave this machine.
*.key
session-signing.key*
# A session's identity belongs to the session, not to the repository. Older
# doors wrote session.id here; nothing reads it now, and it must not be
# committed — several sessions can be live in one work tree at once.
session.id
agent.sock
"#;
...
# Who countersigns session keys and staged artifacts. Set `by` to enable a
# `countersign <target>` step below: the tool stages a needs file naming this
...
# `countersign session` witnesses the session key; `countersign <artifact>`
# witnesses the attestation this session made under that target name.
# [countersign]
# by = "comms.steward:z..."
# role = "guardian"
...
#
# One artifact may have its own witness, distinct from the default party:
...
# `attest config` records the exact bytes of this file under the session key,
# so `status` can report when the rules in force drift from the rules on the
# record: steps = ["mint session", "attest config", "attest entry"]
...
## Sessions are a succession, and history does not un-happen

Every artifact a session produces is filename-scoped by a tag derived from its
steward id, so the door's history is derivable from what it holds — no side
ledger. `status` reads it back as a succession:

```
  sessions on record (2):
    1  comms.steward:z1KbS5CC…  [closed]
        open ✓ complete   archive · not begun   close ✓ complete
    2  comms.steward:zDpoxry3…  [live]
```

Two questions that a session key used to answer at once are now separate:
whether a step **happened** (its signed product is in the store under that
session's tag) and whether **you** can sign as that session (you hold its
seed). Shredding a key ends the second and leaves the first untouched — a
closed session reads as having minted *and* having shredded, which is what
happened. `comms next` therefore only ever offers to open a *new* session; it
never re-offers a completed rite as work, and `status --json` reports each
generation under `sessions[]` with its rites as that session left them.

A session that ended without finishing reads as unfinished, not as absent: the
gap is part of the record.

## Sessions run side by side

Several sessions can be live in one work tree at once — a team of specialists
on one project, each with their own signing identity, without passing commits
around or keeping separate clones. There is no coordination to do: artifacts,
pending items, and bundles are all scoped by a tag derived from the signing
session, so they cannot collide.

**The session id is not stored anywhere.** It is derived from wherever your
seed lives, which is the only place that can answer the question honestly:

| `session_key` | your seed lives in | concurrent? |
|---|---|---|
| `ephemeral`  | `COMMS_SESSION_SEED`, per process | yes |
| `ssh-agent`  | the agent at `COMMS_AGENT_SOCK` / `SSH_AUTH_SOCK` | yes — a per-session socket is a per-session identity |
| `file`       | `<comms>/session.key`, one path | no: one session per door |

A seed found somewhere the config did not ask for is never adopted: in `file`
mode a stray `COMMS_SESSION_SEED` is not this door's session, and signing as it
would be the worst kind of quiet.

Each session is one of three things, from the vantage of whoever is asking:

- **mine** — you hold its seed and can sign as it.
- **open** — minted, with no closing evidence in the store. A peer working
  right now and a session that died without finishing are *indistinguishable*
  from outside, and the door says so rather than guessing.
- **concluded** — its closing rite reached the end of its recorded work.

## Ritual closing and key destruction are different claims

`concluded` says the rite concluded. It says nothing about the seed, and the
naming is deliberate: "closed" invites the inference, and the inference is not
supported.

Every rite verb but one leaves an artifact. `shred` leaves nothing — a
destroyed seed and a carefully kept one are identical in the store, and no
signature can distinguish them. So `shred` is answered only for your own
session, where you can see the answer, and reported **unverifiable** for every
other:

```
    rite close  [concluded]
      ✓ attest transcript
      ✓ seal store
      ? shred session  (unverifiable: only its holder could witness this)
```

That is not a gap waiting to be closed by a better mechanism. It is outside
what a signature can express. A rite reaches its end with the step set aside —
`complete()` does not block on it, because nobody could ever satisfy it — and
`status --json` reports `"status": "unverifiable"` while `"done"` stays false,
so a consumer cannot read completion as proof.

`mine` is checked before `concluded`, and the order is load-bearing: if closing
evidence won, sealing would mark your own shred done and `next` would skip it,
leaving the rite reporting complete while you still hold a live key.

A community that wants a claim about key destruction on the record needs
nothing new here — its session can attest one as its last act
(`attest <target>` like anything else). That is the session's own testimony,
offered rather than derived, and it is the most anyone can honestly provide.

Note what this does **not** solve. Two agents editing one work tree still need
whatever coordination they would have needed anyway — comms scopes the
*attestations*, not the files. And git identity is repo-global: if two sessions
sign commits, pass the identity per command (`git -c user.signingkey=…`) rather
than wiring it into the repo.

## Countersigning is not only for session keys

`countersign session` stages an endorsement of the session key. `countersign
<target>` stages one of the artifact this session attested under that name — a
constitution, an executable interface, a trial-log entry. Declare the party in
`[countersign]`, or give one artifact its own witness with a per-target table:

```toml
[countersign]                     # the default party
by = "comms.steward:z..."
role = "guardian"

[countersign.constitution]        # this one artifact, a different witness
by = "comms.steward:z..."
role = "witness"
```

The step completes only when their signed endorsement of *that* subject has
been finalized into the store.

## Every step reports what it produced

A rite step is an act on the record, and the record of the act should not have
to be recovered from a sentence. Every step prints one machine-readable line
alongside its prose — a fixed `comms-step` prefix and a JSON object, so it is
greppable, `jq`-able, and unambiguous about paths containing spaces:

```
comms-step {"act":"attest","rite":"open","target":"entry","result":"attested",
            "id":"comms.attest:z…","kind":"testimony","about":"entry",
            "body_b3":"72c0cd61…","body_len":"8","signer":"comms.steward:z…",
            "output":"./.comms/store/entry.z6JXrvY6.cbor",
            "session_after":"live","next":"countersign session"}
```

`act` and `result` are always present. The rest are the verb's own facts — for
`attest` the id and the blake3 of the body it commits to, for `seal` the seal's
id and the bundle's hash and member count, for `mint` the session created and
where its seed is held, for `shred` that the seed is gone. `session_after` says
whether the acting session is still live; `next` is that session's next step or
`rite-complete`.

`comms next --json` prints the same record as the whole of its output, for
callers that want nothing else. The ephemeral session seed is never part of it:
it would land in a log the moment anyone redirected the output.

Fields are added over time. Read by key and ignore the rest.

## The rules in force vs. the rules on the record

`attest config` attests the exact bytes of the active `comms.toml` — it takes
no `--body`, because supplying the content by hand would let the attested
document and the one in force differ from the start. `status` then reports
whether the live file still matches the attested copy, and names both hashes
when it does not. A config is meant to change; a config that changed without
anyone noticing is the thing being guarded against.
